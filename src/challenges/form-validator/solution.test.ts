...
type X1<T> = {
  [key in string]: Array<keyof T>;
};

const makeFormValidatorFactory =
  <T extends Record<string, (arg: string) => string | void>>(validators: T) =>
  <Z extends X1<T>>(config: Z) => {
    return (values: Record<keyof Z, string>) => {
      const errors = {} as Record<keyof Z, string | undefined>;
...
// const x: X<{id: string, username: number}> = {
//   id: ['id'],
//   test: ['username']
// };
