...
export interface Cache<T> {
  ...
  clone: <Y>(transform: (elem: T) => Y) => Cache<Y>;
}
...
it("Should let you clone the cache using a transform function 2", () => {
  const numberCache = createCache<boolean>();

  numberCache.set("a", true);
  numberCache.set("b", false);
  ...
  type tests = [Expect<Equal<typeof a, { a: boolean } | undefined>>];
});
